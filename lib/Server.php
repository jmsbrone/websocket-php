...
final class Server implements LoggerAwareInterface, Stringable
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $errno = null;
        $errstr = null;
        set_error_handler(function (int $severity, string $message, string $file, int $line, array $context) use (&$error): bool {
        ...
        if (!$this->listening) {
            $error = sprintf('Could not open listening socket: %s (%s) %s', $errstr, $errno, $error);
    ...
    public function getHeader($header): ?string
    {
        foreach ($this->request as $row) {
            if (false !== stripos((string) $row, (string) $header)) {
                [$headername, $headervalue] = explode(':', (string) $row);
    ...
    {
        return array_any($this->connections, fn ($connection) => $connection->isConnected());
    }

    /**
     * Get name of local socket from single connection.
     *
     * @return null|string name of local socket
     */
     ...
     *
     * @return null|string name of remote socket
     */
    ...
    private function connect(): void
    {
        try {
            ...
            $error = 'Server failed to connect. '.$errorException->getMessage();
